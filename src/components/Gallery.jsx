...
const Gallery = () => {
  ...
  const fetchGalleryImages = async () => {
    try {
      ...
      (await toast).error("Failed to fetch gallery images!");
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("https://frontbis.onrender.com/upload-multiple", {
        method: "POST",
        body: formData,
      });
      const data = await response.json();
      ...
      if (response.ok) {
        (await toast).success("Upload successful!");
        fetchGalleryImages(); // Refresh gallery images
      } else {
        (await toast).error(data.message);
      }
    } catch (error) {
      (await toast).error("Failed to upload images!");
    }
  };
  ...
  const handleDelete = async (imageId) => {
    try {
      const response = await fetch(
        `https://frontbis.onrender.com/delete-image/${imageId}`,
        {
          method: "DELETE",
        }
      );
      ...
      <div style={styles.galleryContainer}>
        {galleryImages.map((image) => (
          <div key={image.id} style={styles.imageWrapper}>
            ...
            <img
              src={`https://frontbis.onrender.com/${image.photo}` || "/noproduct.jpg"}
